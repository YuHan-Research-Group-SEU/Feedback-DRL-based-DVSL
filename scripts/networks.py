...
class Actor(nn.Module):
    ...
    def reset_parameters(self):
         # 对权重进行正交初始化
        self.fc1.weight.data.copy_(torch.Tensor(init.orthogonal_(self.fc1.weight)))
        self.fc2.weight.data.copy_(torch.Tensor(init.orthogonal_(self.fc2.weight)))
...
class Critic(nn.Module):
    ...
    def reset_parameters(self):
        self.fcs1.weight.data.copy_(torch.Tensor(init.orthogonal_(self.fcs1.weight)))
        self.fc2.weight.data.copy_(torch.Tensor(init.orthogonal_(self.fc2.weight)))
        self.fc3.weight.data.uniform_(-3e-1, 3e-1)
        """
